...
def track_white_object(video_path, crop_width, crop_height):
    ...
    while cap.isOpened():
        ...
        if contours:
            ...
            if M["m00"] != 0:  # Avoid division by zero
                ...
                # Append frame number, center, and bounding box coordinates
                positions.append((frame_count, cx, cy, x, y, w, h))
    ...
    df = pd.DataFrame(positions, columns=["Frame", "Center_X", "Center_Y", "Box_X", "Box_Y", "Box_Width", "Box_Height"])
    df.to_csv("tracked_positions.csv", index=False)
    print("Tracking complete! Positions saved to 'tracked_positions_with_bounding_box.csv'.")
